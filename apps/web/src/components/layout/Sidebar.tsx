...
export function Sidebar() {
  ...
  return (
    <aside
      ...
      style={{
        background: 'linear-gradient(180deg, #1e2540 0%, #161d2e 100%)',
    ...
    >
      ...
      {/* Logo */}
      <div
        className={cn('relative z-10 flex items-center gap-3 border-b px-4 py-[18px] shrink-0', collapsed && 'justify-center px-0')}
      ...
      >
        ...
        {!collapsed && (
          <div>
            ...
            <div className="text-[10px] font-medium leading-tight" style={{ color: 'rgba(165,180,252,0.7)' }}>
              AI Lending Platform
            </div>
          </div>
        )}
              ...
              {group.items.map(({ href, icon: Icon, label, badge }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Icon className="w-4 h-4 shrink-0" style={{ color: active ? '#a5b4fc' : 'rgba(255,255,255,0.45)' }} />
                    {!collapsed && (
                      <>
                        <span className="flex-1 leading-none" style={{ color: active ? '#e0e7ff' : 'rgba(255,255,255,0.6)' }}>
                          {label}
                        </span>
                        {badge && (
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                                style={{ background: 'rgba(165,180,252,0.15)', color: '#a5b4fc', border: '1px solid rgba(165,180,252,0.2)' }}>
                            {badge}
                          </span>
                        )}
                      </>
      ...
      {/* User */}
      {!collapsed && (
        <div className="relative z-10 p-3 border-t shrink-0" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
          <div className="flex items-center gap-2.5 p-2 rounded-lg cursor-pointer hover:bg-white/[0.06] transition-colors">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-black text-white shrink-0"
                 style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
              RM
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-semibold text-white/80 truncate leading-tight">Rahul Mehta</div>
              <div className="text-[10px] leading-tight" style={{ color: 'rgba(255,255,255,0.35)' }}>Risk Manager</div>
            </div>
            <Settings className="w-3.5 h-3.5" style={{ color: 'rgba(255,255,255,0.3)' }} />
          </div>
        </div>
      )}
      ...
      <button
        ...
        style={{ background: '#ffffff', border: '1px solid rgba(0,0,0,0.12)', boxShadow: '0 2px 8px rgba(0,0,0,0.12)' }}
      >
